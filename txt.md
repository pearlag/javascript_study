...
.filter("#id") // 특정 노드만을 걸러대고 싶을 때 같은 레벨 안에 있는 것만 찾는다 (자식,자손,부모 상관x)
.find("#id") // 특정 노드만을 걸러대고 싶을 때. 자손 노드를 검색한다.
...
```js
.prev() // 이전 형제 노드
.prevAll() // 이전 형제 노드들 모두
.prevAll(["#id"]) // 이전 형제 노드들 중 특정 노드

.next() // 다음 형제 노드
.nextAll() // 다음 형제 노드들 모두
.nextAll(["#id"]) // 다음 형제 노드들 중 특정 노드
...
// 특정 노드의 이전 위치에 추가
$("ul.menu").before($newItem); 
  // 또는
$newItem.insertBefore($("ul.menu"));

//특정 노드의 다음 위치에 추가
$("ul.menu").after($newItem);
  // 또는
$newItem.insertAfter($("ul.menu"));
```
